...
<head>
<meta charset="utf-8">
<title>WhyHQ's - Incluir HQ's</title>
...
$('#vl_preco').mask('000.000.000.000.000,00', {reverse: true});
$("#numisbn").mask('000-00-000-0000-0');
...
<?php
    ...
    $consultaCat = $cn->query("select * from tbCategoria");
    $consultaAutor = $cn->query("select * from tbAutor");
			...
			<div class="col-sm-4 col-sm-offset-4">
				...
				<form method="post" action="insprod.php" name="incluiProd" enctype="multipart/form-data">
					...
					<select class="form-control" name="sltcat">
					  ...
					  <option value="<?php echo $listaCat['codCategoria'] ?>"><?php echo $listaCat['descrCategoria'] ?></option>					
					<?php } ?>
                    </select>
                    ...
                    <option value="">Selecione</option>
					  ...
					  <option value="<?php echo $listaAutor['codAutor'] ?>"><?php echo $listaAutor['nomeAutor'] ?></option>					
					<?php } ?>
					</select>
